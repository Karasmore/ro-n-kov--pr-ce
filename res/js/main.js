...
const generate = () => {   //draw pipes
    ...
    pipeList.forEach((pipe, i) => {
        ...
        if (pipe.position.x >= 200 && pipe.position.x <= 500) {
...
const collisionPipes = (pipe) => {
    ...
    if (
        ...
        (player.position.x >= pipe.position.x && player.position.x - player.width <= pipe.position.x + pipe.width) && // X Kolize

        !(playerPosition > pipe.position.y + pipe.topCoord - hitbox && playerPosition + player.height < pipe.topCoord + pipe.position.y + 240 + hitbox) // Y kolize
